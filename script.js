...
function vulRooster(state){
  ...
  // groene lijn bij Gemiddeld, pas wanneer placeholder getoond moet worden
if(state.showCommaPlaceholder && state.level === "3"){
  const col = state.decimalQuotPos;
  const startRij = 0; // of de rij waar je de lijn wilt laten starten
  for(let r = startRij; r < rooster.length; r++){
...
if (chk) {
 ...
 document.addEventListener("keydown", (e) => {
  ...
  if(!sel) return;
  let r = Number(sel.dataset.r);
  let c = Number(sel.dataset.c);

  switch(e.key){
    case ",":
    case ".":
      if(!state.showCommaPlaceholder) return;
      state.userDecimalPos = c;
  ...
  // beperk tot roostergrenzen
  r = Math.max(0, Math.min(r, state.rows-1));
  c = Math.max(0, Math.min(c, state.cols-1));

  // update selectie
  UI.selectCell(r, c); // je functie die de visuele selectie zet
  e.preventDefault();
});
...
state.userGrid = maakLeegRooster(state.stappen.length * 2, cols);
  ...
  `${toUI(state.dividend)} ÷ ${toUI(state.divisor)} =`;
